...
st.subheader("""Alguns esclarecimentos referente ao Projeto
             Fonte dos dados: http://dados.prefeitura.sp.gov.br/it/dataset/coleta-de-residuos-solidos-urbanos
    Repositório: https://github.com/rv-dnl/datascience-project/tree/main""")
...
def plot_residuos_por_ano(ano):
    ...
    if df is not None:
        ...
        st.subheader("Gráfico Geral - Resíduos Coletados por Tipo de Resíduo")
        ...
        conclusao = """Conforme análise feita dos anos de 2013 a 2020, observa-se que o lixo domiciliar se mantém como a maior categoria de resíduo coletado no sistema público de limpeza urbana de São Paulo, representando a maior parte do total de resíduos sólidos gerados. No entanto, é importante destacar que, embora esse tipo de resíduo seja predominante, existem flutuações ao longo dos anos que indicam variações no padrão de geração e coleta de resíduos, possivelmente influenciadas por fatores como mudanças no consumo, no comportamento da população e nas políticas públicas implementadas durante esse período.

        A análise também revela que, mesmo com o foco predominante no resíduo domiciliar, a cidade de São Paulo tem enfrentado desafios relacionados ao aumento constante da geração de resíduos, especialmente em áreas urbanas densamente povoadas. Além disso, é possível notar uma leve tendência de crescimento no volume de resíduos coletados, o que pode ser associado ao aumento populacional, ao crescimento do consumo e à maior conscientização sobre a segregação de resíduos recicláveis, embora essa tendência não seja linear.

        Entretanto, é relevante mencionar que os dados não contemplam os resíduos gerados por grandes geradores e pelo setor privado, o que significa que o total de resíduos urbanos gerados na cidade pode ser ainda maior do que os números apresentados. A exclusão dos resíduos do sistema de logística reversa também deixa uma lacuna importante na análise, especialmente considerando o aumento da reciclagem e da reutilização desses materiais ao longo dos anos.

        Portanto, a predominância do lixo domiciliar não deve obscurecer a importância de uma abordagem integrada para a gestão de resíduos, que leve em consideração todos os segmentos da sociedade, incluindo o setor privado e grandes geradores. A implementação de políticas públicas mais eficazes, como a promoção da reciclagem e o incentivo à redução da geração de resíduos, se mostra fundamental para enfrentar os desafios de sustentabilidade e gestão de resíduos na cidade. A continuidade das ações para melhorar a eficiência do sistema de coleta e ampliar a participação de todos na responsabilidade pela destinação adequada do lixo será crucial para os próximos anos."""

        conclusao = "\n".join([linha.lstrip() for linha in conclusao.splitlines()])
